#!/usr/bin/env python3
"""Regenerate a specific standalone panel from the keratinocyte supplementary figure."""
...
def parse_args(panel_required: bool = True, show_panel: bool = True) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Regenerate a specific standalone panel from the keratinocyte supplementary figure."
    )
    parser.add_argument(
        "--panel",
        choices=["E", "F", "G", "H"],
        required=panel_required,
        help="Standalone supplementary panel to render." if show_panel else argparse.SUPPRESS,
    )
    parser.add_argument(
        "--cohort",
...
def run_panel(panel: str, args: argparse.Namespace) -> None:
    ...
    print(f"Regenerating supplementary panel {panel} from: {annotated}")
